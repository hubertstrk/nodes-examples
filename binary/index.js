...
const serialize = ({starred, project, text, deleted}) => {
  const starredBuffer = encodeBoolean(starred)

  const archivedBuffer = encodeBoolean(deleted)
...
const deserialize = (result) => {
  ...
  const archived = buffer[1] === 1 ? true : false
  index += 1
...
for (let i = 0; i < 2000; i++) {
  writeAndRead({
    ...
    archived: starred[Math.floor(Math.random() * 2)],
